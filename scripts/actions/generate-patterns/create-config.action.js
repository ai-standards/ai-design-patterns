...
import { openai, ReasoningEffort, Verbosity } from '../../lib/client.js';

export default async function createConfig(context) {
...
async function extractTags(readmeContent) {
...
${readmeContent}`;
    ...
    try {
        ...
        console.warn('Failed to extract tags, using defaults:', error.message);
        return ['ai-pattern', 'design-pattern', 'implementation'];
    }
}

async function extractCategory(readmeContent) {
...
${readmeContent}`;
    ...
    try {
        ...
        console.warn('Failed to extract category, using default:', error.message);
        return 'Operational';
    }
}
