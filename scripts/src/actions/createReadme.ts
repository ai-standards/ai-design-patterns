...
export default async function createReadme(context: ActionContext): Promise<ActionContext> {
...
Write in a formal yet approachable, first-person style.`;
  ...
  try {
    ...
    createReadmeSpinner.fail(`Failed to generate README: ${(error as Error).message}`);
    throw error;
  }
}
