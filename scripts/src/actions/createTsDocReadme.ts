...
export default async function createTsDocReadme(context: ActionContext): Promise<ActionContext> {
...
Write clear, concise documentation that helps developers understand and use the example.`;
  ...
  try {
    ...
    createTsDocReadmeSpinner.fail(`Failed to generate TypeScript documentation: ${(error as Error).message}`);
    throw error;
  }
}
