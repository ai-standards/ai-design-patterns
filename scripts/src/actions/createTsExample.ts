...
export default async function createTsExample(context: ActionContext): Promise<ActionContext> {
...
The code should be production-ready and follow TypeScript best practices.`;
  ...
  try {
    ...
    createTsExampleSpinner.fail(`Failed to generate TypeScript example: ${(error as Error).message}`);
    throw error;
  }
}
